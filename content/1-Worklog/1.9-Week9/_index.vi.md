...
* Thực hiện fork project mẫu `fcj-lab/aws-fcj-container-app` về tài khoản GitLab cá nhân.
* Chỉnh sửa tệp `frontend/src/components/Menu.jsx` từ `"Dashboard"` thành `"Dashboard v1.0.1"`.

###### 3.2. Cài đặt và Đăng ký GitLab Runner
Khởi chạy một EC2 Instance đóng vai trò làm build agent chạy Ubuntu Server, thực thi cài đặt GitLab Runner:
```bash
# setup.sh
apt update -y
...
```
* Chuyển sang tài khoản hệ thống `gitlab-runner`:
```bash
sudo -i
su gitlab-runner
cd
```
* Tiến hành đăng ký (register) Runner với dự án GitLab:
```bash
gitlab-runner register --url https://gitlab.com --token <YOUR_REGISTRATION_TOKEN>
```
* Chọn Executor là `shell`. Chạy ngầm Runner:
```bash
nohup gitlab-runner run > start-runner-be.txt 2>&1 &
```

###### 3.3. Cấu hình Quyền hạn và IAM Role
* Chạy `sudo visudo` và thêm dòng cấu hình để cấp quyền sudo không mật khẩu cho `gitlab-runner`:
```text
gitlab-runner ALL=(ALL) NOPASSWD:ALL
```
* Khởi tạo IAM Role `ECS-GitLabRunner-Role` tin cậy dịch vụ EC2 và gán cho máy ảo chạy GitLab Runner để có quyền tương tác với ECR và ECS API mà không cần dùng Access Keys tĩnh.
* Minh chứng vai trò IAM Role của Runner trên Console:

![IAM Role Runner](/images/worklog/week-9/1_iam_role_gitlab_runner.png)

###### 3.4. Cấu hình Variables và Chạy thử Pipeline
* Cấu hình các biến CI/CD bảo mật trong Settings dự án GitLab bao gồm: `CLUSTER_NAME`, `SERVICE_NAME_FE`, `SERVICE_NAME_BE`, `REGION`, `AWS_APPLICATION_NAME`, `AWS_DEPLOYMENT_GROUP_NAME`, `ACCOUNT_NUMBER`.
* Cấu hình pipeline chính `.gitlab-ci.yml` phân tách thành các giai đoạn build, push và deploy.

###### 3.5. Kiểm tra Kết quả (Check Results)
* Tạo Git Tag phiên bản mới và push lên GitLab. Quan sát pipeline chạy thành công trên tab CI/CD Pipelines.
* ECS Service sẽ tự động cập nhật task definition revision mới nhất và ứng dụng cập nhật giao diện thành công.

##### 4. Quy trình CI/CD với GitHub Actions

###### 4.1. Clone GitHub Template
* Clone kho lưu trữ template ứng dụng của FCJ về local máy tính.

###### 4.2. Khởi tạo Project và Đẩy mã nguồn lên GitHub
* Tạo repository trống trên tài khoản GitHub cá nhân, đẩy toàn bộ source code ứng dụng lên nhánh `main`.
...
* Đẩy một tag mới lên GitHub. GitHub Actions sẽ tự động kích hoạt, đóng gói container backend/frontend đẩy lên ECR, tự động tạo cấu hình AppSpec và kích hoạt CodeDeploy thay đổi phiên bản của ứng dụng.

##### 5. Quy trình CI/CD với AWS CodeBuild
Sử dụng công cụ CI/CD native của AWS.

###### 5.1. Cấu hình liên kết GitHub
* Truy cập AWS CodeBuild Console, thực hiện liên kết kết nối tài khoản AWS với tài khoản GitHub thông qua GitHub OAuth hoặc Personal Access Token (PAT).

###### 5.2. Khởi tạo dự án CodeBuild cho Frontend
* Tạo build project `Frontend-Build`. Sử dụng file cấu hình `buildspec.yml` nằm trong thư mục nguồn Frontend để mô tả các bước cài đặt dependencies, build project và đóng gói Docker.
